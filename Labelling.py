...
End_button = Button(label='Finish', button_type = 'danger', height= 100)
...
if show_table:
    right_side = gridplot([End_button, table], ncols=1)
else:
    right_side = End_button
# curdoc().add_root(column(Good_button ,Med_button ,Bad_button ,Double_button ,tc_button , p))
# curdoc().add_root(p)
curdoc().add_root(row(plot, right_side))
